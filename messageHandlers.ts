 ...
 */
function wsSendWithSession(ws: ServerWebSocket<ChatWebSocketData>, payload: Record<string, unknown>, sessionId?: string): void {
  const sid = sessionId || ws.data?.sessionId;
  const message = sid ? { ...payload, sessionId: sid } : payload;
...
async function handleChatMessage(
  ...
  if (!content || !sessionId) {
    wsSendWithSession(ws, { type: 'error', error: 'Missing content or sessionId' }, sessionId as string | undefined);
    return;
  }
...
async function handleApprovePlan(
  ...
  if (!sessionId) {
    wsSendWithSession(ws, { type: 'error', error: 'Missing sessionId' });
    return;
  }
...
async function handleSetPermissionMode(
  ...
  if (!sessionId || !mode) {
    wsSendWithSession(ws, { type: 'error', error: 'Missing sessionId or mode' }, sessionId as string | undefined);
    return;
  }
...
async function handleKillBackgroundProcess(
  ...
  if (!bashId) {
    wsSendWithSession(ws, { type: 'error', error: 'Missing bashId' }, sessionId as string | undefined);
    return;
  }
