...
export const debugLog = (message: string): void => {
  if (IS_STANDALONE) {
    try {
      const logPath = path.join(path.dirname(process.execPath), 'agentic-debug.log');
...
export const getBinaryDir = (): string => {
  // import.meta.dir gives us /path/to/agentic/server
  // We need to go up one level to /path/to/agentic (the app root)
