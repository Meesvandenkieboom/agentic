...
export function MessageList({ messages, isLoading, liveTokenCount = 0, scrollContainerRef }: MessageListProps) {
  ...
  useEffect(() => {
    ...
    if (isBulkLoad) {
      ...
      scrollCooldownRef.current = false;
      setIsAtBottom(true);
      // Use virtualizer.scrollToIndex for reliable scroll with virtual lists
      // Then fall back to manual scrollTop after virtualizer measures
      requestAnimationFrame(() => {
        virtualizer.scrollToIndex(messages.length - 1, { align: 'end' });
        // Secondary scroll after virtualizer measures actual heights
        setTimeout(() => {
          if (container) {
  ...
  return (
    <div
      ref={parentRef}
      className="flex overflow-y-auto overflow-x-hidden z-10 flex-col flex-1 pb-2.5 w-full max-w-full min-h-0 scrollbar-hidden"
    >
